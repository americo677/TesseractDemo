...
- (BOOL)moveTessdataToDirectoryIfNecessary:(NSString *)directoryPath
    {
        ...
        // Se obtiene el path del bundle sin adjuntar la carpeta referenciada tessdata
        NSString *tessdataPath = [[NSBundle mainBundle] resourcePath];
        ...
        // AJCG nuevas lineas para filtrar los archivos del bundle por la extensión
        NSPredicate *predicate = [NSPredicate predicateWithFormat:@"SELF contains[c] '.traineddata'"];
        ...
        if (files == nil) {
            ...
            for (NSString *filename in files) {
                ...
                if (![fileManager fileExistsAtPath:destinationFileName]) {
                    ...
                    // Se suprime la eliminación del enlace simbolico original tomado del bundle
                    ...
                    // AJCG Se reemplaza la creación de enlace simbolico por copia del archivo a la carpeta destino
                    //BOOL res = [fileManager createSymbolicLinkAtPath:destinationFileName withDestinationPath:filePath error:&error];
                    BOOL res = [fileManager copyItemAtPath:destinationFileName toPath:filePath error:&error];
